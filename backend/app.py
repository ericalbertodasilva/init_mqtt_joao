...
def create_app(config_name):
    ...
    def save_registers():
        ...
        result = register.save_product(request.form)
        if result:
            message = "Inserido"
        else:
    ...
    def update_registers():
        register = RegisterController()
        result = register.update_product(request.form)
        if result:
            message = "Editado"
        else:
            message = "Não Editado"
    ...
    @app.route('/registers/<limit>', methods=['GET'])
    @auth_token_required
    ...
    def get_product(product_id):
        ...
        register = RegisterController()
        response = register.get_product_by_id(product_id = product_id)
        return Response(json.dumps(response, ensure_ascii=False), mimetype='application/json'), response['status'], header
